...
BOT_TOKEN = "8747913075:AAGbado2fT729wQI9lhY8_WxiiASviSXAck"
...
_shop_bot_poll_lock = None


def launch_shop_bot_if_enabled() -> None:
    """Start polling in a daemon thread. Call from whichever WSGI module loads (main or api_server)."""
    global _shop_bot_poll_lock
    if os.getenv("ENABLE_SHOP_BOT", "1").strip().lower() in ("0", "false", "no"):
        logger.info("Shop Telegram bot disabled (ENABLE_SHOP_BOT=0)")
        return
    _ensure_bot_data()
    lock_path = os.path.join(DATA_DIR, "shop_bot_poll.lock")
    try:
        import fcntl

        _shop_bot_poll_lock = open(lock_path, "a+", encoding="utf-8")
        fcntl.flock(_shop_bot_poll_lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    except BlockingIOError:
        logger.info(
            "Shop Telegram bot skipped — another Gunicorn worker already holds shop_bot_poll.lock"
        )
        return
    except ImportError:
        logger.warning(
            "fcntl unavailable (non-Unix); if you run multiple Gunicorn workers, set workers=1"
        )
    except OSError as e:
        logger.warning("Could not acquire shop bot poll lock (%s); starting bot anyway", e)
    try:
        run_bot_thread()
        logger.info("Shop Telegram bot thread started")
    except Exception:
        logger.exception("Failed to start shop Telegram bot")
